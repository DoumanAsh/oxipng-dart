...
class _FilePickerDemoState extends State<FilePickerDemo> {
  ...
  Future<void> _saveFile() async {
    try {
      ...
      print("file list=$_paths");
      if (_paths?.first != null) {
          final image = _paths?.first.path;
          final imageFile = File(image!);
          final bytes = await imageFile.readAsBytes();

          print("bytes_len=${bytes.length}");
          try {
              final optimizedBytes = await _oxipngPlugin.optimizeFromMemory(bytes);
              final outputFile = File(fileName!);
              await outputFile.writeAsBytes(optimizedBytes!);
          } catch (error) {
              print("Cannot optimize: $error");
          }
      }
